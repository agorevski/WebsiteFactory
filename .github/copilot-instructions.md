...
`examples/**/website.yaml` -> `@website-factory/schema` Zod validation -> optional normalized v2 content -> `@website-factory/generator` signals/plans -> template/theme/component registries -> `@website-factory/website-builder` Astro static routes -> `apps/website-builder/dist` -> optional QA/deployment plan tooling.
...
- Keep generated outputs and local state out of commits: `node_modules`, `dist`, `.astro`, `.vite`, coverage, `*.tsbuildinfo`, `.env*`, provider CLI folders, and generated QA artifacts.
...
- `@website-factory/seo`: metadata, JSON-LD, sitemap/robots/RSS artifacts, and schema adapters.
...
- `@website-factory/website-builder`: Astro demo/preview app, example discovery, static routes, template prototypes, QA surface, and final static output.
...
- `theme.name` may target a registered theme from `packages/themes/src/themes.ts`; keep `theme.palette` for schema/backward compatibility.
...
- The builder recursively discovers `examples/**/website.yaml`, parses each site, sorts by business name, and uses Astro `getStaticPaths()` for root and nested static routes.
...
- Generated-site QA scripts exist for deeper checks: `npm run qa:generated:static`, `npm run qa:generated:html`, `npm run qa:generated:playwright`, `npm run qa:generated:lhci`, `npm run qa:generated`, and `npm run qa:generated:report`.
