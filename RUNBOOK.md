...
| Page template | Use only schema-supported `pages[].template` values: `landing` or `service-index`. Root pages usually use `landing`; service directories use `service-index`. Do not use composition template IDs from the registry/prototype catalog here. | `packages/schema/src/index.ts`; app route mapping in `apps/website-builder/src/lib/pages.ts`. |
