...
export class WebsiteValidationError extends Error {
    ...
    source;
    formatted;
    constructor(message, { issues = [], source = "schema", cause, } = {}) {
        super(message, { cause });
...
export function safeValidateWebsiteData(data) {
    try {
...
export function parseWebsiteYaml(yaml) {
    let parsed;
    try {
        parsed = parseYaml(yaml);
    }
    catch (error) {
        throw new WebsiteValidationError(`Website YAML could not be parsed: ${getErrorMessage(error)}`, { source: "yaml", cause: error });
    }
    return validateWebsiteData(parsed);
}
export function parseUniversalSite(yaml, sourcePath = "website.yaml") {
    let parsed;
    try {
        parsed = parseYaml(yaml);
    }
    catch (error) {
        throw new Error(`Invalid universal site YAML in ${sourcePath}: ${getErrorMessage(error)}`, { cause: error });
    }
...
export function safeParseWebsiteYaml(yaml) {
    try {
...
export function formatWebsiteValidationError(error) {
    return formatWebsiteValidationIssues(error.issues);
}
export function formatWebsiteValidationIssues(issues) {
    if (issues.length === 0) {
        return "Website data is invalid, but no validation issues were reported.";
    }
    const lines = issues.map((issue) => `- ${formatPath(issue.path)}: ${issue.message} (${issue.code})`);
    return [
...
function toWebsiteValidationError(error) {
